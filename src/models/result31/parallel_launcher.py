...
from models.result31.runner import (
    ...
    enumerate_result31_tasks_from_config,
...
def _task_weight(task: dict[str, str]) -> float:
    ...
    return (
        ...
        * timeline_weight.get(str(task["timeline"]), 1.0)
...
def shard_tasks(
    ...
    n_workers: int,
    weight_fn: Callable[[dict[str, str]], float] | None = None,
    ...
    scorer = weight_fn or _task_weight
...
def _launch_workers(
    *,
    ...
    for worker_idx, cfg_path in enumerate(worker_cfg_paths):
        ...
        cmd = [
            sys.executable,
            "scripts/run_result3_1_fullseason_hcg_multitrait.py",
            "--config",
            str(cfg_path),
        ]
...
def merge_worker_outputs(
    *,
    ...
    if not feature_overview_df.empty and "timeline" in feature_overview_df.columns:
        feature_overview_df = feature_overview_df.drop_duplicates(subset=["timeline"], keep="first")
    ...
    if not split_usage_df.empty:
        split_usage_df = split_usage_df.drop_duplicates().reset_index(drop=True)
    ...
    if not metrics_fold_df.empty:
        metrics_fold_df = metrics_fold_df.drop_duplicates(
            subset=["scenario", "timeline", "target", "predictor", "target_year", "outer_fold"],
            keep="first",
        )
    ...
    if not oof_df.empty:
        oof_df = oof_df.drop_duplicates(
            subset=["plot_id", "scenario", "timeline", "target", "predictor", "target_year", "outer_fold"],
            keep="first",
        )
    ...
    if not inner_pred_df.empty:
        inner_pred_df = inner_pred_df.drop_duplicates(
            subset=["plot_id", "scenario", "timeline", "target", "predictor", "target_year", "outer_fold", "inner_fold"],
            keep="first",
        )
    ...
    if not trial_df.empty:
        trial_df = trial_df.drop_duplicates(
            subset=["scenario", "timeline", "target", "predictor", "target_year", "outer_fold", "trial_number"],
            keep="first",
        )
...
def run_result3_1_parallel_launcher(config_path: Path, *, n_workers: int) -> Path:
    ...
    tasks = enumerate_result31_tasks_from_config(cfg)
    if not tasks:
    ...
    print(f"Result3.1 parallel launcher completed: {output_dir}", flush=True)
    return output_dir


def main() -> None:
    parser = argparse.ArgumentParser(description="Parallel launcher for Result3.1 task-level multiprocessing.")
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--n-workers", type=int, required=True)
    args = parser.parse_args()
    run_result3_1_parallel_launcher(Path(args.config), n_workers=int(args.n_workers))
